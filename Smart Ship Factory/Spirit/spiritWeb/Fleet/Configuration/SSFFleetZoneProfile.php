<?php
//Include Common Files @1-AE3A2124
define("RelativePath", "../..");
define("PathToCurrentPage", "/Fleet/Configuration/");
define("FileName", "SSFFleetZoneProfile.php");
...
//Operation Method @6-27B012B7
    function Operation()
    {
        ...
        $Redirect = "SSFFleetZoneProfile.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "SSFFleetZoneProfile.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Class_Initialize Event @3-930CB644
    function clsGridssf_flt_product_profile($RelativePath, & $Parent)
    {
        ...
        $this->pp_profile_id->Page = "SSFFleetZoneProfile.php";
        ...
        $this->ssf_flt_profile_Insert->Page = "SSFFleetZoneProfile.php";
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->pp_profile_id->Parameters = CCGetQueryString("QueryString", array("pp_tkt_plu_id", "ccsForm"));
                $this->pp_profile_id->Parameters = CCAddParam($this->pp_profile_id->Parameters, "pp_profile_id", $this->DataSource->f("zp_profile_id"));
...
//SetOrder Method @3-E698BBCF
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            array("Sorter_pp_profile_id" => array("zp_profile_id", "")));
    }
//End SetOrder Method

//Prepare Method @3-91794EEF
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opContains, "zp_profile_id", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @3-86AFF487
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(DISTINCT( zp_profile_id)) \n\n" .
        "FROM ssf_flt_zone_profile";
        $this->SQL = "SELECT zp_profile_id \n\n" .
        "FROM ssf_flt_zone_profile {SQL_Where}\n\n" .
        "GROUP BY zp_profile_id {SQL_OrderBy}";
...
//SetValues Method @3-3469493A
    function SetValues()
    {
        $this->pp_profile_id->SetDBValue($this->f("zp_profile_id"));
    }
...
//Class_Initialize Event @16-C439930E
...
//CheckErrors Method @16-8CA09847
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @16-08958061
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @16-8764CBAF
    function clsssf_flt_product_profile1DataSource(& $Parent)
    {
        ...
        $this->InsertFields["zp_profile_id"] = array("Name" => "zp_profile_id", "Value" => "", "DataType" => ccsText);
        $this->InsertFields["zp_station_id"] = array("Name" => "zp_station_id", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["zp_profile_id"] = array("Name" => "zp_profile_id", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @16-E6B9B37A
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "zp_profile_id", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @16-42034261
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM ssf_flt_zone_profile {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->PageSize = 1;
        $this->query($this->OptimizeSQL(CCBuildSQL($this->SQL, $this->Where, $this->Order)));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
...
//SetValues Method @16-FE6444C3
    function SetValues()
    {
        $this->pp_profile_id->SetDBValue($this->f("zp_profile_id"));
        $this->pp_tkt_plu_id->SetDBValue($this->f("zp_station_id"));
    }
//End SetValues Method

//Insert Method @16-ED38D60C
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->InsertFields["zp_profile_id"]["Value"] = $this->pp_profile_id->GetDBValue(true);
        $this->InsertFields["zp_station_id"]["Value"] = $this->pp_tkt_plu_id->GetDBValue(true);
        $this->SQL = CCBuildInsert("ssf_flt_zone_profile", $this->InsertFields, $this);
...
//Update Method @16-46E7FC35
    function Update()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["zp_profile_id"] = new clsSQLParameter("ctrlpp_profile_id", ccsText, "", "", $this->pp_profile_id->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["zp_profile_id"]->GetValue()) and !strlen($this->cp["zp_profile_id"]->GetText()) and !is_bool($this->cp["zp_profile_id"]->GetValue())) 
            $this->cp["zp_profile_id"]->SetValue($this->pp_profile_id->GetValue(true));
        $wp->Criterion[1] = $wp->Operation(opEqual, "zp_profile_id", $wp->GetDBValue("1"), $this->ToSQL($wp->GetDBValue("1"), ccsText),false);
        $Where = 
             $wp->Criterion[1];
        $this->UpdateFields["zp_profile_id"]["Value"] = $this->cp["zp_profile_id"]->GetDBValue(true);
        $this->SQL = CCBuildUpdate("ssf_flt_zone_profile", $this->UpdateFields, $this);
...
//Delete Method @16-99BDDA85
    function Delete()
    {
        ...
        $Where = 
             $wp->Criterion[1];
        $this->SQL = "DELETE FROM ssf_flt_zone_profile";
        $this->SQL = CCBuildSQL($this->SQL, $Where, "");
...
//Class_Initialize Event @42-454D7345
    function clsGridssf_flt_product_profile_s($RelativePath, & $Parent)
    {
        ...
        $this->pp_tkt_plu_id->Page = "SSFFleetZoneProfile.php";
        ...
        $this->ssf_flt_product_Insert->Page = "SSFFleetZoneProfile.php";
        $this->Navigator = new clsNavigator($this->ComponentName, "Navigator", $FileName, 10, tpCentered, $this);
    }
//End Class_Initialize Event
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->pp_tkt_plu_id->Parameters = CCAddParam($this->pp_tkt_plu_id->Parameters, "pp_profile_id", $this->DataSource->f("zp_profile_id"));
                $this->pp_tkt_plu_id->Parameters = CCAddParam($this->pp_tkt_plu_id->Parameters, "pp_tkt_plu_id", $this->DataSource->f("zp_station_id"));
                $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeShowRow", $this);
                $this->Attributes->Show();
                $this->pp_tkt_plu_id->Show();
                $Tpl->block_path = $ParentPath . "/" . $GridBlock;
...
//DataSource Variables @42-E4020246
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @42-FB0CDF74
...
//SetOrder Method @42-41FDFE10
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "zp_station_id";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            array("Sorter_pp_tkt_plu_id" => array("zp_station_id", "")));
    }
//End SetOrder Method

//Prepare Method @42-AA6697D0
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//Class_Initialize Event @58-61C72E54
...
//CheckErrors Method @58-16B4FD5F
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @58-4493D94C
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @58-E0F6F09A
    function clsssf_flt_product_profile2DataSource(& $Parent)
    {
        ...
        $this->InsertFields["zp_station_id"] = array("Name" => "zp_station_id", "Value" => "", "DataType" => ccsText);
        $this->InsertFields["zp_profile_id"] = array("Name" => "zp_profile_id", "Value" => "", "DataType" => ccsText);
        $this->UpdateFields["zp_station_id"] = array("Name" => "zp_station_id", "Value" => "", "DataType" => ccsText);
        $this->UpdateFields["zp_profile_id"] = array("Name" => "zp_profile_id", "Value" => "", "DataType" => ccsText);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @58-F491CFB5
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "zp_profile_id", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
        $this->wp->Criterion[2] = $this->wp->Operation(opEqual, "zp_station_id", $this->wp->GetDBValue("2"), $this->ToSQL($this->wp->GetDBValue("2"), ccsText),false);
...
//SetValues Method @58-523D677D
    function SetValues()
    {
        $this->s_pp_tkt_plu_id->SetDBValue($this->f("zp_station_id"));
        $this->pp_profile_id->SetDBValue($this->f("zp_profile_id"));
    }
//End SetValues Method

//Insert Method @58-2A3421A8
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildInsert", $this->Parent);
        $this->InsertFields["zp_station_id"]["Value"] = $this->s_pp_tkt_plu_id->GetDBValue(true);
        $this->InsertFields["zp_profile_id"]["Value"] = $this->pp_profile_id->GetDBValue(true);
        $this->SQL = CCBuildInsert("ssf_flt_zone_profile", $this->InsertFields, $this);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteInsert", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
...
//Update Method @58-B9380028
    function Update()
    {
        ...
        $this->CmdExecution = true;
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildUpdate", $this->Parent);
        $this->UpdateFields["zp_station_id"]["Value"] = $this->s_pp_tkt_plu_id->GetDBValue(true);
        $this->UpdateFields["zp_profile_id"]["Value"] = $this->pp_profile_id->GetDBValue(true);
        $this->SQL = CCBuildUpdate("ssf_flt_zone_profile", $this->UpdateFields, $this);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "SSFFleetZoneProfile.html";
$BlockToParse = "main";
...
//Include events file @1-7992F100
include("./SSFFleetZoneProfile_events.php");
