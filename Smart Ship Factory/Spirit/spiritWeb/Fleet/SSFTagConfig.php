<?php
//Include Common Files @1-13A1CA24
define("RelativePath", "..");
define("PathToCurrentPage", "/Fleet/");
...
//Class_Initialize Event @5-96701CAC
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->tkt_tag_id->Parameters = CCAddParam($this->tkt_tag_id->Parameters, "tkt_tag_id", $this->DataSource->f("ftd_tag_id"));
...
//SetOrder Method @3-0B4C615D
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "ftd_tag_id";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            array("Sorter_tkt_tag_id" => array("ftd_tag_id", "")));
    }
//End SetOrder Method

//Prepare Method @3-5E4EC6BC
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opContains, "ftd_tag_id", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @3-779C1C88
    function Open()
    {
...
//SetValues Method @3-74278AB9
    function SetValues()
    {
        $this->tkt_tag_id->SetDBValue($this->f("ftd_tag_id"));
    }
...
//Class_Initialize Event @20-8D6F74A6
    function clsRecordssf_tkt_customers_tags1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->tkt_tag_fleet_model_id = new clsControl(ccsTextBox, "tkt_tag_fleet_model_id", "tkt_tag_fleet_model_id", ccsText, "", CCGetRequestParam("tkt_tag_fleet_model_id", $Method, NULL), $this);
            $this->Link1 = new clsControl(ccsLink, "Link1", "Link1", ccsText, "", CCGetRequestParam("Link1", $Method, NULL), $this);
            $this->Link1->Page = "SSFPopFleetModelList.php";
...
//Validate Method @20-A33073BA
    function Validate()
    {
        ...
        $Validation = ($this->tkt_tag_fleet_model_id->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->tkt_tag_id->Errors->Count() == 0);
        $Validation =  $Validation && ($this->tkt_tag_fleet_model_id->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @20-1CDC235E
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->tkt_tag_id->Errors->Count());
        $errors = ($errors || $this->tkt_tag_fleet_model_id->Errors->Count());
        $errors = ($errors || $this->Link1->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
...
//InsertRow Method @20-B68344C8
    function InsertRow()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeInsert", $this);
        if(!$this->InsertAllowed) return false;
        $this->DataSource->tkt_tag_id->SetValue($this->tkt_tag_id->GetValue(true));
        $this->DataSource->tkt_tag_fleet_model_id->SetValue($this->tkt_tag_fleet_model_id->GetValue(true));
    ...
    function Show()
    {
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    $this->tkt_tag_id->SetValue($this->DataSource->tkt_tag_id->GetValue());
                    $this->tkt_tag_fleet_model_id->SetValue($this->DataSource->tkt_tag_fleet_model_id->GetValue());
                }
            } else {
                $this->EditMode = false;
            }
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->tkt_tag_fleet_model_id->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Link1->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Errors->ToString());
        ...
        $this->tkt_tag_id->Show();
        $this->tkt_tag_fleet_model_id->Show();
        $this->Link1->Show();
...
//DataSource Variables @20-EA5EE0AB
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
    ...
    // Datasource fields
    public $tkt_tag_id;
    public $tkt_tag_fleet_model_id;
    public $Link1;
//End DataSource Variables

//DataSourceClass_Initialize Event @20-5C3C3E36
    function clsssf_tkt_customers_tags1DataSource(& $Parent)
    {
        ...
        $this->Initialize();
        $this->tkt_tag_id = new clsField("tkt_tag_id", ccsText, "");
        $this->tkt_tag_fleet_model_id = new clsField("tkt_tag_fleet_model_id", ccsText, "");
        $this->Link1 = new clsField("Link1", ccsText, "");

        $this->InsertFields["ftd_tag_id"] = array("Name" => "ftd_tag_id", "Value" => "", "DataType" => ccsText);
        $this->InsertFields["ftd_tag_fleet_model_id"] = array("Name" => "ftd_tag_fleet_model_id", "Value" => "", "DataType" => ccsText);
        $this->UpdateFields["ftd_tag_id"] = array("Name" => "ftd_tag_id", "Value" => "", "DataType" => ccsText);
        $this->UpdateFields["ftd_tag_fleet_model_id"] = array("Name" => "ftd_tag_fleet_model_id", "Value" => "", "DataType" => ccsText);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @20-22979A90
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "ftd_tag_id", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @20-C9786FC2
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
...
//SetValues Method @20-8F4E82A9
    function SetValues()
    {
        $this->tkt_tag_id->SetDBValue($this->f("ftd_tag_id"));
        $this->tkt_tag_fleet_model_id->SetDBValue($this->f("ftd_tag_fleet_model_id"));
    }
//End SetValues Method

//Insert Method @20-AF5222CB
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->InsertFields["ftd_tag_id"]["Value"] = $this->tkt_tag_id->GetDBValue(true);
        $this->InsertFields["ftd_tag_fleet_model_id"]["Value"] = $this->tkt_tag_fleet_model_id->GetDBValue(true);
...
//Update Method @20-CD83F63D
    function Update()
    {
        ...
        $this->UpdateFields["ftd_tag_id"]["Value"] = $this->tkt_tag_id->GetDBValue(true);
        $this->UpdateFields["ftd_tag_fleet_model_id"]["Value"] = $this->tkt_tag_fleet_model_id->GetDBValue(true);
