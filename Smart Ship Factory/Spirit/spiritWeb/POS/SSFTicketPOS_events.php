...
//Custom Code @27-2A29BDB7
	$Component->SetValue("pos_config_id") ;
//End Custom Code
...
//Custom Code @65-2A29BDB7
	global $Redirect ;
	$Redirect = "SSFTicketPOS.php?tkt_catalog_code=".$ssf_tkt_catalog1->tkt_catalog_code->GetValue() ;
	$db = new clsDBConnection1();
	$db2 = new clsDBConnection1();
	$sqlQuery = "select tkt_catalog_code from ssf_tkt_catalog where tkt_catalog_id = 'pos_param_config'";
	$db->query($sqlQuery);
	while($db->next_record())
	{
		$sqlInsert = "insert into ssf_tkt_configuration values('".$ssf_tkt_catalog1->tkt_catalog_code->GetValue()."','".$db->f("tkt_catalog_code")."','')";
		$db2->query($sqlInsert);
	}
	$db->close();
	$db2->close();
...
// -------------------------
	...
	if( $thisParam == "POS_ACTIVE" || $thisParam == "PLU_SPEEDKEYS" || $thisParam == "PAY_SPEEDKEYS" ||
		$thisParam == "PRINTER_ID" || $thisParam == "RECEIPT_PRINT_INMEDIATELY" || $thisParam == "PERIOD_CLOSE_GROUP" ||
		( ($thisParam == "RECEIPT_GROUP" || $thisParam == "RECEIPT_GROUP_CREDIT") && CCGetSession("SSFCustomizationGroup") == "GUATEMALA001" ) )
		$Component->Visible = false;
	else
...
// -------------------------
	...
	if( $thisParam == "RECEIPT_PRINT_INMEDIATELY")
	{
		$arrayOfValues[0] = array("N",$CCSLocales->GetText("ssf_print_inmediately_n"));
		$arrayOfValues[1] = array("Y",$CCSLocales->GetText("ssf_print_inmediately_y"));
	}
	if( $thisParam == "PRINTER_ID")
	{
		$sqlQuery = "select distinct tkt_printer_id from ssf_tkt_printers";
		$db->query($sqlQuery);
		$arrayIndex = 0;
		while($db->next_record()) {
			$arrayOfValues[$arrayIndex] = array($db->f("tkt_printer_id"),$db->f("tkt_printer_id"));
			$arrayIndex++ ;
		}
	}
	if( $thisParam == "PLU_SPEEDKEYS")
	{
		$sqlQuery = "select distinct tkt_catalog_code from ssf_tkt_catalog where tkt_catalog_id = 'speedkey_group_id' and tkt_catalog_misc1 = 'PLU_SPEEDKEYS'";
		$db->query($sqlQuery);
		$arrayIndex = 0;
		while($db->next_record()) {
			$arrayOfValues[$arrayIndex] = array($db->f("tkt_catalog_code"),$db->f("tkt_catalog_code"));
			$arrayIndex++ ;
		}
	}
	if( $thisParam == "PAY_SPEEDKEYS")
	{
		$sqlQuery = "select distinct tkt_catalog_code from ssf_tkt_catalog where tkt_catalog_id = 'speedkey_group_id' and tkt_catalog_misc1 = 'PAY_SPEEDKEYS'";
		$db->query($sqlQuery);
		$arrayIndex = 0;
		while($db->next_record()) {
			$arrayOfValues[$arrayIndex] = array($db->f("tkt_catalog_code"),$db->f("tkt_catalog_code"));
	...
	if( $thisParam == "PERIOD_CLOSE_GROUP")
	{
		$sqlQuery = "select tkt_period_group_id,tkt_period_group_desc from ssf_tkt_period_groups ";
		$db->query($sqlQuery);
		$arrayIndex = 0;
		while($db->next_record()) {
			$arrayOfValues[$arrayIndex] = array($db->f("tkt_period_group_id"),$db->f("tkt_period_group_desc"));
			$arrayIndex++ ;
		}
	}
...
//Page_AfterInitialize @1-0482655F
function Page_AfterInitialize(& $sender)
{
    $Page_AfterInitialize = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $SSFTicketPOS; //Compatibility
...
//Custom Code @28-2A29BDB7
	...
	$EditisAllowed = $accmgr->CheckUserAction("WEB_TKT_POS_EDIT") ;
	$ViewisAllowed = $accmgr->CheckUserAction("WEB_TKT_POS_VIEW") ;
