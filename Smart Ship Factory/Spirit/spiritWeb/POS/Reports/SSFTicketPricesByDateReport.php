<?php
//Include Common Files @1-5A52F82A
define("RelativePath", "../..");
define("PathToCurrentPage", "/POS/Reports/");
define("FileName", "SSFTicketPricesByDateReport.php");
...
class clsRecordssf_tkt_pricesSearch { //ssf_tkt_pricesSearch Class @3-2FCD91BC
...
//Class_Initialize Event @3-6DA6864D
...
"FROM ssf_tkt_catalog {SQL_Where} {SQL_OrderBy}";
            ...
            $this->Link1->Page = "SSFTicketPricesByDateReport.php";
            $this->Button_DoSearch = new clsButton("Button_DoSearch", $Method, $this);
        }
    }
//End Class_Initialize Event

//Validate Method @3-A05FABB1
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
...
//Operation Method @3-9E81BD22
    function Operation()
    {
        ...
        $Redirect = "SSFTicketPricesByDateReport.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "SSFTicketPricesByDateReport.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
} //End ssf_tkt_pricesSearch Class @3-FCB6E20C

//ssf_tkt_prices_ssf_tkt_pl ReportGroup class @21-44D13F24
class clsReportGroupssf_tkt_prices_ssf_tkt_pl {
    public $GroupType;
    public $mode; //1 - open, 2 - close
    public $Report_TotalRecords, $_Report_TotalRecordsAttributes;
    public $tkt_price_date, $tkt_price_dateDup, $_tkt_price_dateAttributes;
    ...
    public $RowNumber;
    public $Parent;
    public $tkt_price_dateTotalIndex;

    function clsReportGroupssf_tkt_prices_ssf_tkt_pl(& $parent) {
        $this->Parent = & $parent;
    ...
    function SetControls($PrevGroup = "") {
        ...
        if ($PrevGroup) {
            $this->tkt_price_dateDup =  CCCompareValues($this->tkt_price_date, $PrevGroup->tkt_price_date, $this->Parent->tkt_price_date->DataType) == 0;
        }
    }
...
//End ssf_tkt_prices_ssf_tkt_pl ReportGroup class

//ssf_tkt_prices_ssf_tkt_pl GroupsCollection class @21-165E4F3F
class clsGroupsCollectionssf_tkt_prices_ssf_tkt_pl {
    public $Groups;
    public $mPageCurrentHeaderIndex;
    public $mtkt_price_dateCurrentHeaderIndex;
    public $PageSize;
    public $TotalPages = 0;
    public $TotalRows = 0;
    ...
    function clsGroupsCollectionssf_tkt_prices_ssf_tkt_pl(& $parent) {
        $this->Parent = & $parent;
        $this->Groups = array();
        $this->Pages  = array();
        $this->mtkt_price_dateCurrentHeaderIndex = 1;
        $this->mReportTotalIndex = 0;
        $this->mPageTotalIndex = 1;
    }

    function & InitGroup() {
        $group = new clsReportGroupssf_tkt_prices_ssf_tkt_pl($this->Parent);
        $group->RowNumber = $this->TotalRows + 1;
        $group->PageNumber = $this->TotalPages;
        $group->PageTotalIndex = $this->mPageCurrentHeaderIndex;
        $group->tkt_price_dateTotalIndex = $this->mtkt_price_dateCurrentHeaderIndex;
        return $group;
    }
    ...
    function OpenGroup($groupName) {
        ...
        if ($groupName == "tkt_price_date") {
            $Grouptkt_price_date = & $this->InitGroup(true);
            $this->Parent->tkt_price_date_Header->CCSEventResult = CCGetEvent($this->Parent->tkt_price_date_Header->CCSEvents, "OnInitialize", $this->Parent->tkt_price_date_Header);
            if ($this->Parent->Page_Footer->Visible) 
                $OverSize = $this->Parent->tkt_price_date_Header->Height + $this->Parent->Page_Footer->Height;
            else
                $OverSize = $this->Parent->tkt_price_date_Header->Height;
            if (($this->PageSize > 0) and $this->Parent->tkt_price_date_Header->Visible and ($this->CurrentPageSize + $OverSize > $this->PageSize)) {
                $this->ClosePage();
                $this->OpenPage();
            }
            if ($this->Parent->tkt_price_date_Header->Visible)
                $this->CurrentPageSize = $this->CurrentPageSize + $this->Parent->tkt_price_date_Header->Height;
                $Grouptkt_price_date->SetTotalControls("GetNextValue");
            $this->Parent->tkt_price_date_Header->CCSEventResult = CCGetEvent($this->Parent->tkt_price_date_Header->CCSEvents, "OnCalculate", $this->Parent->tkt_price_date_Header);
            $Grouptkt_price_date->SetControls();
            $Grouptkt_price_date->Mode = 1;
            $Grouptkt_price_date->GroupType = "tkt_price_date";
            $this->mtkt_price_dateCurrentHeaderIndex = count($this->Groups);
            $this->Groups[] = & $Grouptkt_price_date;
        }
    }
    ...
    function CloseGroup($groupName)
    {
        ...
        $Grouptkt_price_date = & $this->InitGroup(true);
        $this->Parent->tkt_price_date_Footer->CCSEventResult = CCGetEvent($this->Parent->tkt_price_date_Footer->CCSEvents, "OnInitialize", $this->Parent->tkt_price_date_Footer);
        if ($this->Parent->Page_Footer->Visible) 
            $OverSize = $this->Parent->tkt_price_date_Footer->Height + $this->Parent->Page_Footer->Height;
        else
            $OverSize = $this->Parent->tkt_price_date_Footer->Height;
        if (($this->PageSize > 0) and $this->Parent->tkt_price_date_Footer->Visible and ($this->CurrentPageSize + $OverSize > $this->PageSize)) {
            $this->ClosePage();
            $this->OpenPage();
        }
        $Grouptkt_price_date->SetTotalControls("GetPrevValue");
        $Grouptkt_price_date->SyncWithHeader($this->Groups[$this->mtkt_price_dateCurrentHeaderIndex]);
        if ($this->Parent->tkt_price_date_Footer->Visible)
            $this->CurrentPageSize = $this->CurrentPageSize + $this->Parent->tkt_price_date_Footer->Height;
        $this->Parent->tkt_price_date_Footer->CCSEventResult = CCGetEvent($this->Parent->tkt_price_date_Footer->CCSEvents, "OnCalculate", $this->Parent->tkt_price_date_Footer);
        $Grouptkt_price_date->SetControls();
        $this->RestoreValues();
        $Grouptkt_price_date->Mode = 2;
        $Grouptkt_price_date->GroupType ="tkt_price_date";
        $this->Groups[] = & $Grouptkt_price_date;
    }
...
//End ssf_tkt_prices_ssf_tkt_pl GroupsCollection class

class clsReportssf_tkt_prices_ssf_tkt_pl { //ssf_tkt_prices_ssf_tkt_pl Class @21-D21D15FE

//ssf_tkt_prices_ssf_tkt_pl Variables @21-55BD0A98
    ...
    public $tkt_price_date_HeaderBlock, $tkt_price_date_Header;
    public $tkt_price_date_FooterBlock, $tkt_price_date_Footer;
    ...
    public $tkt_price_date_HeaderControls, $tkt_price_date_FooterControls;
    public $Sorter_tkt_price_time;
    public $Sorter_tkt_plu_id;
    public $Sorter_tkt_plu_long_desc;
    public $Sorter_tkt_price_price;
//End ssf_tkt_prices_ssf_tkt_pl Variables

//Class_Initialize Event @21-7C1A177F
    function clsReportssf_tkt_prices_ssf_tkt_pl($RelativePath = "", & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "ssf_tkt_prices_ssf_tkt_pl";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        ...
        $this->tkt_price_date_Footer = new clsSection($this);
        $this->tkt_price_date_Header = new clsSection($this);
        $this->Errors = new clsErrors();
        $this->DataSource = new clsssf_tkt_prices_ssf_tkt_plDataSource($this);
        ...
        $this->SorterName = CCGetParam("ssf_tkt_prices_ssf_tkt_plOrder", "");
        $this->SorterDirection = CCGetParam("ssf_tkt_prices_ssf_tkt_plDir", "");
...
//CheckErrors Method @21-29A1D9E7
    function CheckErrors()
    {
        $errors = false;
...
//Show Method @21-27B3A156
    function Show()
    {
        ...
        $tkt_price_dateKey = "";
        $Groups = new clsGroupsCollectionssf_tkt_prices_ssf_tkt_pl($this);
        ...
        while($is_next_record) {
            ...
            if (count($Groups->Groups) == 2 or $tkt_price_dateKey != $this->DataSource->f("tkt_price_date")) {
                $Groups->OpenGroup("tkt_price_date");
            }
            $Groups->AddItem();
            $tkt_price_dateKey = $this->DataSource->f("tkt_price_date");
            $is_next_record = $this->DataSource->next_record();
            if (!$is_next_record || $tkt_price_dateKey != $this->DataSource->f("tkt_price_date")) {
                $Groups->CloseGroup("tkt_price_date");
            }
        }
            ...
            do {
                ...
                switch ($items[$i]->GroupType) {
                    Case "":
                        ...
                        $this->tkt_price_date->Visible = $this->ControlsVisible["tkt_price_date"] && !$items[$i]->tkt_price_dateDup;
                    ...
                    case "tkt_price_date":
                        if ($items[$i]->Mode == 1) {
                            $this->tkt_price_date_Header->CCSEventResult = CCGetEvent($this->tkt_price_date_Header->CCSEvents, "BeforeShow", $this->tkt_price_date_Header);
                            if ($this->tkt_price_date_Header->Visible) {
                                $Tpl->block_path = $ParentPath . "/" . $ReportBlock . "/Section tkt_price_date_Header";
                                $this->Attributes->Show();
                                $Tpl->block_path = $ParentPath . "/" . $ReportBlock;
                                $Tpl->parseto("Section tkt_price_date_Header", true, "Section Detail");
                            }
                        }
                        if ($items[$i]->Mode == 2) {
                            $this->tkt_price_date_Footer->CCSEventResult = CCGetEvent($this->tkt_price_date_Footer->CCSEvents, "BeforeShow", $this->tkt_price_date_Footer);
                            if ($this->tkt_price_date_Footer->Visible) {
                                $Tpl->block_path = $ParentPath . "/" . $ReportBlock . "/Section tkt_price_date_Footer";
                                $this->Attributes->Show();
                                $Tpl->block_path = $ParentPath . "/" . $ReportBlock;
                                $Tpl->parseto("Section tkt_price_date_Footer", true, "Section Detail");
                            }
                        }
                        break;
                }
...
} //End ssf_tkt_prices_ssf_tkt_pl Class @21-FCB6E20C

class clsssf_tkt_prices_ssf_tkt_plDataSource extends clsDBConnection1 {  //ssf_tkt_prices_ssf_tkt_plDataSource Class @21-AD7B36B8

//DataSource Variables @21-0E880714
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @21-276EC910
    function clsssf_tkt_prices_ssf_tkt_plDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Report ssf_tkt_prices_ssf_tkt_pl";
        $this->Initialize();
...
//SetOrder Method @21-C71B5DF8
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            array("Sorter_tkt_price_time" => array("tkt_price_time", ""), 
...
//Open Method @21-D252C8F8
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT tkt_price_date, tkt_price_time, tkt_price_price, tkt_plu_id, tkt_plu_long_desc \n\n" .
        ...
        $this->query(CCBuildSQL($this->SQL, $this->Where, "ssf_tkt_prices.tkt_price_date asc" .  ($this->Order ? ", " . $this->Order: "")));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
//End Open Method

//SetValues Method @21-B31F6992
    function SetValues()
    {
...
} //End ssf_tkt_prices_ssf_tkt_plDataSource Class @21-FCB6E20C

//Initialize Page @1-34D1938E
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "SSFTicketPricesByDateReport.html";
$BlockToParse = "main";
...
//Include events file @1-B06BCC7A
include("./SSFTicketPricesByDateReport_events.php");
...
$ssf_tkt_prices_ssf_tkt_pl = new clsReportssf_tkt_prices_ssf_tkt_pl("", $MainPage);
$MainPage->SSFSpiritHeader = & $SSFSpiritHeader;
$MainPage->ssf_tkt_pricesSearch = & $ssf_tkt_pricesSearch;
$MainPage->Button1 = & $Button1;
$MainPage->ViewMode = & $ViewMode;
$MainPage->ssf_tkt_prices_ssf_tkt_pl = & $ssf_tkt_prices_ssf_tkt_pl;
$ssf_tkt_prices_ssf_tkt_pl->Initialize();
...
//Go to destination page @1-F482D19D
if($Redirect)
{
    ...
    unset($ssf_tkt_prices_ssf_tkt_pl);
    unset($Tpl);
    exit;
}
//End Go to destination page

//Show Page @1-546B3C18
$SSFSpiritHeader->Show();
$ssf_tkt_pricesSearch->Show();
$Button1->Show();
$ssf_tkt_prices_ssf_tkt_pl->Show();
$ViewMode->Show();
$Tpl->block_path = "";
