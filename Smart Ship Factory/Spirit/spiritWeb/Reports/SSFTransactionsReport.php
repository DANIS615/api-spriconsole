...
//Class_Initialize Event @3-961C47FC
    function clsRecordssf_pump_salesSearch($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->s_time_from_tmp = new clsControl(ccsTextBox, "s_time_from_tmp", "s_time_from_tmp", ccsDate, array("HH", ":", "nn", ":", "ss"), CCGetRequestParam("s_time_from_tmp", $Method, NULL), $this);
            $this->s_time_from = new clsControl(ccsHidden, "s_time_from", "s_time_from", ccsText, "", CCGetRequestParam("s_time_from", $Method, NULL), $this);
            $this->s_time_to_tmp = new clsControl(ccsTextBox, "s_time_to_tmp", "s_time_to_tmp", ccsDate, array("HH", ":", "nn", ":", "ss"), CCGetRequestParam("s_time_to_tmp", $Method, NULL), $this);
            $this->s_time_to = new clsControl(ccsHidden, "s_time_to", "s_time_to", ccsText, "", CCGetRequestParam("s_time_to", $Method, NULL), $this);
...
//Validate Method @3-A04FF5F5
    function Validate()
    {
        ...
        $Validation = ($this->s_time_from_tmp->Validate() && $Validation);
        $Validation = ($this->s_time_from->Validate() && $Validation);
        $Validation = ($this->s_time_to_tmp->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->s_time_from_tmp->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_time_from->Errors->Count() == 0);
        $Validation =  $Validation && ($this->s_time_to_tmp->Errors->Count() == 0);
...
//CheckErrors Method @3-D9CEB984
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->s_time_from_tmp->Errors->Count());
        $errors = ($errors || $this->s_time_from->Errors->Count());
        $errors = ($errors || $this->s_time_to_tmp->Errors->Count());
...
//Show Method @3-1258DD65
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->s_time_from_tmp->Errors->ToString());
            $Error = ComposeStrings($Error, $this->s_time_from->Errors->ToString());
            $Error = ComposeStrings($Error, $this->s_time_to_tmp->Errors->ToString());
        ...
        $this->s_time_from_tmp->Show();
        $this->s_time_from->Show();
        $this->s_time_to_tmp->Show();
        $this->s_time_to->Show();
        $this->s_pump_id->Show();
...
//Variables @179-8C2FBB64
    ...
    public $Sorter_ppu;
    public $Sorter_money;
    public $Sorter_initial_volume;
    public $Sorter_final_volume;
    public $Sorter_sale_type;
    public $Sorter_pay_payment_type;
//End Variables

//Class_Initialize Event @179-B9BF1044
    function clsGridconfig_values_ssf_addin_p($RelativePath, & $Parent)
    {
        ...
        $this->volume = new clsControl(ccsLabel, "volume", "volume", ccsFloat, array(False, 3, Null, "", False, "", "", 1, True, ""), CCGetRequestParam("volume", ccsGet, NULL), $this);
        ...
        $this->Data_initial_volume = new clsPanel("Data_initial_volume", $this);
        $this->initial_volume = new clsControl(ccsLabel, "initial_volume", "initial_volume", ccsFloat, array(False, 2, Null, "", False, "", "", 1, True, ""), CCGetRequestParam("initial_volume", ccsGet, NULL), $this);
        $this->Data_final_volume = new clsPanel("Data_final_volume", $this);
        $this->final_volume = new clsControl(ccsLabel, "final_volume", "final_volume", ccsFloat, array(False, 2, Null, "", False, "", "", 1, True, ""), CCGetRequestParam("final_volume", ccsGet, NULL), $this);
        ...
        $this->Header_initial_volume = new clsPanel("Header_initial_volume", $this);
        $this->Sorter_initial_volume = new clsSorter($this->ComponentName, "Sorter_initial_volume", $FileName, $this);
        ...
        $this->Header_final_volume = new clsPanel("Header_final_volume", $this);
        $this->Sorter_final_volume = new clsSorter($this->ComponentName, "Sorter_final_volume", $FileName, $this);
        ...
        $this->Header_initial_volume->AddComponent("Sorter_initial_volume", $this->Sorter_initial_volume);
        $this->Header_initial_volume->AddComponent("col12", $this->col12);
        $this->Header_final_volume->AddComponent("Sorter_final_volume", $this->Sorter_final_volume);
        $this->Header_final_volume->AddComponent("col13", $this->col13);
        ...
        $this->Data_initial_volume->AddComponent("initial_volume", $this->initial_volume);
        $this->Data_final_volume->AddComponent("final_volume", $this->final_volume);
...
//Show Method @179-AB019719
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["Data_initial_volume"] = $this->Data_initial_volume->Visible;
            $this->ControlsVisible["initial_volume"] = $this->initial_volume->Visible;
            $this->ControlsVisible["Data_final_volume"] = $this->Data_final_volume->Visible;
            $this->ControlsVisible["final_volume"] = $this->final_volume->Visible;
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->initial_volume->SetValue($this->DataSource->initial_volume->GetValue());
                $this->final_volume->SetValue($this->DataSource->final_volume->GetValue());
                ...
                $this->Data_initial_volume->Show();
                $this->Data_final_volume->Show();
        ...
        $this->col9->Show();
        $this->Header_initial_volume->Show();
        $this->Sorter_initial_volume->Show();
        $this->col12->Show();
        $this->Header_final_volume->Show();
        $this->Sorter_final_volume->Show();
        $this->col13->Show();
...
//GetErrors Method @179-2E7E476A
    function GetErrors()
    {
        ...
        $errors = ComposeStrings($errors, $this->initial_volume->Errors->ToString());
        $errors = ComposeStrings($errors, $this->final_volume->Errors->ToString());
...
//DataSource Variables @179-000F4B22
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @179-ECF5CDDB
    function clsconfig_values_ssf_addin_pDataSource(& $Parent)
    {
        ...
        $this->initial_volume = new clsField("initial_volume", ccsFloat, "");
        $this->final_volume = new clsField("final_volume", ccsFloat, "");
...
//SetOrder Method @179-CA55C27F
    function SetOrder($SorterName, $SorterDirection)
    {
        ...
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            ...
            "Sorter_initial_volume" => array("initial_volume", ""), 
            "Sorter_final_volume" => array("final_volume", ""), 
            "Sorter_sale_type" => array("sale_type", ""), 
            "Sorter_pay_payment_type" => array("pay_payment_type", "")));
    }
//End SetOrder Method

//Prepare Method @179-3D18B013
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->AddParameter("3", "urlp_sale_start", ccsInteger, "", "", $this->Parameters["urlp_sale_start"], 0, false);
        $this->wp->AddParameter("4", "urls_grade_id", ccsInteger, "", "", $this->Parameters["urls_grade_id"], 0, false);
        $this->wp->AddParameter("5", "urls_pump_id", ccsInteger, "", "", $this->Parameters["urls_pump_id"], 0, false);
        $this->wp->AddParameter("6", "urls_sale_type", ccsInteger, "", "", $this->Parameters["urls_sale_type"], 0, false);
        $this->wp->AddParameter("7", "urls_unit_id", ccsText, "", "", $this->Parameters["urls_unit_id"], "", false);
        $this->wp->AddParameter("8", "urls_time_from", ccsText, "", "", $this->Parameters["urls_time_from"], "", false);
        $this->wp->AddParameter("9", "urls_time_to", ccsText, "", "", $this->Parameters["urls_time_to"], "", false);
        $this->wp->AddParameter("10", "urlp_sale_end", ccsInteger, "", "", $this->Parameters["urlp_sale_end"], 0, false);
    }
//End Prepare Method

//Open Method @179-A655044F
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT sale_id, end_date, end_time, pump_id, hose_id, grade_id, volume, money, ppu, sale_type, pay_payment_type, config_values.id AS id, initial_volume, final_volume\n" .
        "FROM (ssf_pump_sales INNER JOIN ssf_addin_payments_data ON\n" .
        "ssf_pump_sales.sale_id = ssf_addin_payments_data.pay_sale_id) INNER JOIN (config_values INNER JOIN config_values config_values1 ON\n" .
        "config_values.library = config_values1.library AND config_values.id = config_values1.id AND config_values.groupname = config_values1.groupname) ON\n" .
        "ssf_pump_sales.grade_id = config_values.param_value\n" .
        "WHERE ( ssf_pump_sales.end_date >= '" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "' or '" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "' = '' )\n" .
        "AND ( ssf_pump_sales.end_date <= '" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "' or '" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "' = '' )\n" .
        "AND ( ssf_pump_sales.sale_id >= " . $this->SQLValue($this->wp->GetDBValue("3"), ccsInteger) . " or " . $this->SQLValue($this->wp->GetDBValue("3"), ccsInteger) . " = 0 )\n" .
        "AND ( (ssf_pump_sales.sale_id <= " . $this->SQLValue($this->wp->GetDBValue("10"), ccsInteger) . " and " . $this->SQLValue($this->wp->GetDBValue("10"), ccsInteger) . " > 0) or " . $this->SQLValue($this->wp->GetDBValue("10"), ccsInteger) . " = 0 )\n" .
        "AND ( ssf_pump_sales.grade_id = " . $this->SQLValue($this->wp->GetDBValue("4"), ccsInteger) . " or " . $this->SQLValue($this->wp->GetDBValue("4"), ccsInteger) . " = 0 )\n" .
        "AND ( ssf_pump_sales.pump_id = " . $this->SQLValue($this->wp->GetDBValue("5"), ccsInteger) . " or " . $this->SQLValue($this->wp->GetDBValue("5"), ccsInteger) . " = 0 )\n" .
        "AND ( ssf_pump_sales.sale_type = " . $this->SQLValue($this->wp->GetDBValue("6"), ccsInteger) . " or " . $this->SQLValue($this->wp->GetDBValue("6"), ccsInteger) . " = 0 )\n" .
        "AND ( config_values.library = 'grade'  )\n" .
        "AND ( config_values.groupname = 'general' )\n" .
        "AND ( config_values.parameter = 'number' )\n" .
        "AND ( config_values1.parameter = 'vol_unit_desc' )\n" .
        "AND ( config_values1.param_value = '" . $this->SQLValue($this->wp->GetDBValue("7"), ccsText) . "' or '" . $this->SQLValue($this->wp->GetDBValue("7"), ccsText) . "' = '' )\n" .
        "AND ( ( ssf_pump_sales.end_date SQLConcat ssf_pump_sales.end_time >= ('" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "' SQLConcat '" . $this->SQLValue($this->wp->GetDBValue("8"), ccsText) . "') )\n" .
        " OR  '" . $this->SQLValue($this->wp->GetDBValue("8"), ccsText) . "' = ''  )\n" .
        "AND ( ( ssf_pump_sales.end_date SQLConcat ssf_pump_sales.end_time <= ('" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "' SQLConcat '" . $this->SQLValue($this->wp->GetDBValue("9"), ccsText) . "') ) \n" .
        " OR  '" . $this->SQLValue($this->wp->GetDBValue("9"), ccsText) . "' = ''  )) cnt";
        $this->SQL = "SELECT sale_id, end_date, end_time, pump_id, hose_id, grade_id, volume, money, ppu, sale_type, pay_payment_type, config_values.id AS id, initial_volume, final_volume\n" .
        "FROM (ssf_pump_sales INNER JOIN ssf_addin_payments_data ON\n" .
        "ssf_pump_sales.sale_id = ssf_addin_payments_data.pay_sale_id) INNER JOIN (config_values INNER JOIN config_values config_values1 ON\n" .
        "config_values.library = config_values1.library AND config_values.id = config_values1.id AND config_values.groupname = config_values1.groupname) ON\n" .
        "ssf_pump_sales.grade_id = config_values.param_value\n" .
        ...
        " OR  '" . $this->SQLValue($this->wp->GetDBValue("8"), ccsText) . "' = ''  )\n" .
        "AND ( ( ssf_pump_sales.end_date SQLConcat ssf_pump_sales.end_time <= ('" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "' SQLConcat '" . $this->SQLValue($this->wp->GetDBValue("9"), ccsText) . "') ) \n" .
        " OR  '" . $this->SQLValue($this->wp->GetDBValue("9"), ccsText) . "' = ''  ){SQL_OrderBy}";
...
//SetValues Method @179-7AD93D7C
    function SetValues()
    {
        ...
        $this->initial_volume->SetDBValue(trim($this->f("initial_volume")));
        $this->final_volume->SetDBValue(trim($this->f("final_volume")));
