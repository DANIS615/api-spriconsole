<?php
//Include Common Files @1-AC038C0C
define("RelativePath", "..");
define("PathToCurrentPage", "/Operation/");
define("FileName", "SSFTankDeepReading.php");
...
//Class_Initialize Event @3-27D7B898
    function clsEditableGridEditableGrid1($RelativePath, & $Parent)
    {
        ...
        $this->lbl_height = new clsControl(ccsLabel, "lbl_height", "lbl_height", ccsText, "", NULL, $this);
        $this->lbl_volume = new clsControl(ccsLabel, "lbl_volume", "lbl_volume", ccsText, "", NULL, $this);
        $this->lbl_tank_id = new clsControl(ccsLabel, "lbl_tank_id", $CCSLocales->GetText("tank_id"), ccsText, "", NULL, $this);
        $this->tank_id = new clsControl(ccsHidden, "tank_id", $CCSLocales->GetText("tank_id"), ccsText, "", NULL, $this);
        $this->actual_prod_height = new clsControl(ccsLabel, "actual_prod_height", "actual_prod_height", ccsText, "", NULL, $this);
        ...
        $this->water_height = new clsControl(ccsTextBox, "water_height", $CCSLocales->GetText("water_height"), ccsText, "", NULL, $this);
        $this->deep_option = new clsControl(ccsRadioButton, "deep_option", "deep_option", ccsText, "", NULL, $this);
        $this->deep_option->DSType = dsListOfValues;
        $this->deep_option->Values = array(array("1", ""), array("2", ""));
        $this->deep_option->HTML = true;
        $this->Button_Submit = new clsButton("Button_Submit", $Method, $this);
        $this->Button_tank_charge = new clsButton("Button_tank_charge", $Method, $this);
...
//GetFormParameters Method @3-6860A447
    function GetFormParameters()
    {
        for($RowNumber = 1; $RowNumber <= $this->TotalRows; $RowNumber++)
        {
            ...
            $this->FormParameters["water_height"][$RowNumber] = CCGetFromPost("water_height_" . $RowNumber, NULL);
            $this->FormParameters["deep_option"][$RowNumber] = CCGetFromPost("deep_option_" . $RowNumber, NULL);
        }
    }
//End GetFormParameters Method

//Validate Method @3-13FEC8BA
    function Validate()
    {
        $Validation = true;
        ...
        for($this->RowNumber = 1; $this->RowNumber <= $this->TotalRows; $this->RowNumber++)
        {
            ...
            $this->water_height->SetText($this->FormParameters["water_height"][$this->RowNumber], $this->RowNumber);
            $this->deep_option->SetText($this->FormParameters["deep_option"][$this->RowNumber], $this->RowNumber);
...
//ValidateRow Method @3-7EF7303B
    function ValidateRow()
    {
        global $CCSLocales;
        $this->tank_id->Validate();
        $this->prod_height->Validate();
        $this->water_height->Validate();
        $this->deep_option->Validate();
        ...
        $errors = ComposeStrings($errors, $this->deep_option->Errors->ToString());
        $this->tank_id->Errors->Clear();
        $this->prod_height->Errors->Clear();
        $this->water_height->Errors->Clear();
        $this->deep_option->Errors->Clear();
...
//CheckInsert Method @3-00693CDB
    function CheckInsert()
    {
        ...
        $filed = ($filed || strlen($this->FormParameters["water_height"][$this->RowNumber]));
        $filed = ($filed || strlen($this->FormParameters["deep_option"][$this->RowNumber]));
        return $filed;
    }
...
//Operation Method @3-57A55681
    function Operation()
    {
        ...
        if($this->Button_Submit->Pressed) {
            $this->PressedButton = "Button_Submit";
        } else if($this->Button_tank_charge->Pressed) {
            $this->PressedButton = "Button_tank_charge";
        ...
        } else if($this->PressedButton == "Button_tank_charge") {
            if(!CCGetEvent($this->Button_tank_charge->CCSEvents, "OnClick", $this->Button_tank_charge) || !$this->UpdateGrid()) {
                $Redirect = "";
            }
...
//Show Method @3-7484099A
    function Show()
    {
        ...
        $this->Button_tank_charge->Visible = $this->Button_tank_charge->Visible && ($this->InsertAllowed || $this->UpdateAllowed || $this->DeleteAllowed);
        ...
        $this->ControlsVisible["actual_prod_height"] = $this->actual_prod_height->Visible;
        ...
        $this->ControlsVisible["deep_option"] = $this->deep_option->Visible;
        if ($is_next_record || ($EmptyRowsLeft && $this->InsertAllowed)) {
            do {
                ...
                if (!($this->FormSubmitted) && $is_next_record) {
                    ...
                    $this->actual_prod_height->SetText("");
                ...
                $this->actual_prod_height->Show($this->RowNumber);
                $this->prod_vol->Show($this->RowNumber);
                $this->prod_height->Show($this->RowNumber);
                $this->water_height->Show($this->RowNumber);
                $this->deep_option->Show($this->RowNumber);
        ...
        $Tpl->block_path = $EditableGridPath;
        if(!is_array($this->lbl_height->Value) && !strlen($this->lbl_height->Value) && $this->lbl_height->Value !== false)
            $this->lbl_height->SetText($CCSLocales->GetText("ssf_height"));
        if(!is_array($this->lbl_volume->Value) && !strlen($this->lbl_volume->Value) && $this->lbl_volume->Value !== false)
            $this->lbl_volume->SetText($CCSLocales->GetText("volume"));
        $this->Sorter_tank_id->Show();
        $this->Sorter_prod_height->Show();
        $this->Sorter_water_height->Show();
        $this->lbl_height->Show();
        $this->lbl_volume->Show();
        $this->Button_Submit->Show();
        $this->Button_tank_charge->Show();
        $this->Cancel->Show();
...
//DataSource Variables @3-75AD3475
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @3-44E5695F
    function clsEditableGrid1DataSource(& $Parent)
    {
        ...
        $this->actual_prod_height = new clsField("actual_prod_height", ccsText, "");
        $this->prod_vol = new clsField("prod_vol", ccsFloat, "");
        $this->prod_height = new clsField("prod_height", ccsText, "");
        $this->water_height = new clsField("water_height", ccsText, "");
        $this->deep_option = new clsField("deep_option", ccsText, "");

    }
//End DataSourceClass_Initialize Event

//SetOrder Method @3-1F021C03
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "SSFTankDeepReading.html";
$BlockToParse = "main";
...
//Include events file @1-D454DF1F
include("./SSFTankDeepReading_events.php");
...
$lblNote = new clsControl(ccsLabel, "lblNote", "lblNote", ccsText, "", CCGetRequestParam("lblNote", ccsGet, NULL), $MainPage);
$MainPage->SSFSpiritHeader = & $SSFSpiritHeader;
$MainPage->EditableGrid1 = & $EditableGrid1;
$MainPage->lblNote = & $lblNote;
